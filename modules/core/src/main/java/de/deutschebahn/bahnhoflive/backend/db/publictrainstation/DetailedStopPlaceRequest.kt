...
class DetailedStopPlaceRequest(
    ...
    override fun parseNetworkResponse(response: NetworkResponse?): Response<DetailedStopPlace> {
        ...
        return try {
            ...
            detailedStopPlace.errorCode?.let {
                Response.error(DetailedVolleyError(this, Exception("Received error code $it")))
            } ?: kotlin.run {
                ...
                Response.success(
                    detailedStopPlace,
                    forcedCacheEntryFactory.createCacheEntry(response)
                )
            }
        } catch (e: Exception) {
